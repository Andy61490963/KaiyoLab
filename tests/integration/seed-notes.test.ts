...
const migration = await readFile(new URL('../../db/migrations/006_engineering_notes.sql', import.meta.url), 'utf8');
...
describe.skipIf(!source)('owner-requested article seed', () => {
  ...
  it('inserts five full public snapshots with actual dates and usable taxonomies', async () => {
    ...
    expect((await client.query('SELECT count(*)::int AS n FROM taxonomies')).rows[0].n).toBeGreaterThan(5);
  ...
  it('never overwrites edits, republishes drafts, restores trash, or changes dates', async () => {
    await client.query(migration);
    const rows = await snapshot();
    await client.query("UPDATE entries SET content = jsonb_set(content, '{body}', '\"My edited body\"'), version = 7 WHERE id=$1", [rows[0].id]);
    ...
    expect((await client.query('SELECT * FROM taxonomies ORDER BY id')).rows).toEqual(taxonomyBefore);
  });
