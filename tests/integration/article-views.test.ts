...
describe.skipIf(!source)('persistent article views without schema changes', () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query('INSERT INTO settings(id,value) VALUES(1,$1)', [{...defaultSettings, homeIntro:'# Original'}]);
    const article = {...emptyContent, title:'Unchanged title', slug:'public', body:'## Body\n\nOriginal'};
    await pool.query(`INSERT INTO entries(id,kind,content,published,published_at) VALUES
      ('public','article',$1,$1,now()),('private','article',jsonb_set($1,'{slug}','"private"'),NULL,NULL),('project','project',$1,$1,now())`, [article]);
    await pool.query(`INSERT INTO entries(id,kind,content,published,deleted_at) VALUES ('trash','article',$1,$1,now())`, [article]);
  ...
  it('serializes concurrent increments without changing publishing versions or content', async () => {
    ...
    const results = await Promise.all(Array.from({length:24}, () => views.incrementArticleViews('public')));
    ...
    try { expect(await views.getArticleViews('public',other)).toBe(24); } finally { await other.end(); }
  });
  it('preserves counters during concurrent admin settings saves and hides them in settings reads', async () => {
    ...
    const save = () => api.ALL!({request:new Request(`${origin}/api/admin/settings`,{
      method:'PUT',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({...defaultSettings,homeIntro:'# Keep my edits', [ARTICLE_VIEWS_KEY]:{public:999999}}),
    }),params:{path:'settings'},url:new URL(`${origin}/api/admin/settings`)} as unknown as APIContext);
    await Promise.all([save(),save(),...Array.from({length:12},()=>views.incrementArticleViews('public'))]);
  ...
  it('API validates Origin, methods, payloads, visibility and signed cooldown receipts', async () => {
    ...
    const call = async (id: string, method='POST', originHeader=origin, payload?:string) => {
      const request = new Request(`${origin}/api/article-views/${id}`,{
        method,headers:{Origin:originHeader},body:payload,
      });
      return await route.ALL!({request,params:{id},url:new URL(request.url),cookies:{
  ...
  it('does not reset corrupt counts or expose errors, and unavailable telemetry is isolated', async () => {
    await pool.query('UPDATE settings SET value=jsonb_set(value,ARRAY[$1,$2],$3::jsonb) WHERE id=1', [ARTICLE_VIEWS_KEY,'public','"bad-count"']);
    await expect(views.incrementArticleViews('public')).rejects.toThrow('Invalid article view count');
    expect((await content.getPublished('article','public'))?.title).toBe('Unchanged title');
    const response = await route.ALL!({request:new Request(`${origin}/api/article-views/public`),
      params:{id:'public'},url:new URL(origin)} as unknown as APIContext) as Response;
