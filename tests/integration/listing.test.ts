...
describe.skipIf(!process.env.DATABASE_URL)('PostgreSQL list pagination and ordering', () => {
  ...
  beforeAll(async () => {
    ...
    await database
      .getPool()
      .query(`INSERT INTO entries(id,kind,content) VALUES('draft','article',$1)`, [
        JSON.stringify({ ...emptyContent, title: 'Private draft', slug: 'draft' }),
      ]);
    await database
      .getPool()
      .query(
        `INSERT INTO entries(id,kind,content,published,deleted_at) VALUES('trash','article',$1,$1,now())`,
        [JSON.stringify({ ...emptyContent, title: 'Trash', slug: 'trash' })],
      );
    await database
      .getPool()
      .query(`INSERT INTO settings(id,value) VALUES(1,$1)`, [
        JSON.stringify({ avatar: '/media/media-25.webp' }),
      ]);
  });
  afterAll(async () => {
  ...
  it('sorts all public results before slicing and preserves snapshot privacy', async () => {
    ...
    expect(result.items.map((item) => item.title)).toEqual(
      Array.from({ length: 8 }, (_, i) => `Note ${String(i + 9).padStart(2, '0')}`),
    );
  ...
  it('uses admin draft titles for global sorting, and filters before counting', async () => {
    const result = await listing.listAdminEntries(
      new URLSearchParams('kind=article&status=published&sort=title-asc&pageSize=10&page=2'),
    );
    expect(result).toMatchObject({ total: 25, page: 2, pages: 3, from: 11, to: 20 });
    expect(result.items[0].content.title).toBe('Private 11');
    expect(
      (await listing.listAdminEntries(new URLSearchParams('status=draft'))).items.map((e) => e.id),
    ).toEqual(['draft']);
    expect(
      (await listing.listAdminEntries(new URLSearchParams('status=trash'))).items.map((e) => e.id),
    ).toEqual(['trash']);
  ...
  it('has stable ID tie-breakers across page boundaries', async () => {
    await database
      .getPool()
      .query(`UPDATE entries SET updated_at='2026-01-01' WHERE published IS NOT NULL`);
    const first = await listing.listAdminEntries(
      new URLSearchParams('status=published&sort=updated-desc&pageSize=20'),
    );
    const second = await listing.listAdminEntries(
      new URLSearchParams('status=published&sort=updated-desc&pageSize=20&page=2'),
    );
  ...
  it('paginates media with global size ordering and accurate usage', async () => {
    ...
    expect(first.items[0]).toMatchObject({
      name: 'Image 25.png',
      usedBy: ['Site settings / About me'],
    });
    const second = await listing.listAdminMedia(
      new URLSearchParams('sort=size-desc&pageSize=12&page=2'),
    );
  ...
  it('normalizes invalid input and clamps a page after the last item is deleted', async () => {
    const invalid = await listing.listAdminEntries(
      new URLSearchParams('sort=title;DROP TABLE entries&pageSize=Infinity&page=-1'),
    );
