...
describe('local draft recovery', () => {
  const valid = {
    content: { ...emptyContent, body: '# Private work', tags: ['C#'] },
  ...
  it.each([null, '', '{broken', 'null', '[]', '42'])('ignores malformed storage: %s', (raw) => {
    expect(readRecovery(raw)).toBeNull();
  });
  it('rejects obsolete shapes and invalid dates, versions, and field types', () => {
    for (const invalid of [
      { ...valid, content: { title: 'Old draft' } },
      { ...valid, at: 'not a date' },
      { ...valid, version: 0 },
      { ...valid, version: 1.5 },
      { ...valid, content: { ...valid.content, body: null } },
      { ...valid, content: { ...valid.content, tags: [42] } },
      { ...valid, content: { ...valid.content, featured: 'true' } },
    ])
