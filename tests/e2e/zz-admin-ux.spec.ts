...
test.beforeAll(async ({ browser, baseURL }) => {
  ...
  try {
    const response = await context.request.post('/api/auth/sign-in/email', {
      headers: { Origin: baseURL! }, data: { email: process.env.E2E_EMAIL || 'e2e@example.test', password: process.env.E2E_PASSWORD || 'KaiyoLab-e2e-password-2026' },
    });
    expect(response.ok(), await response.text()).toBe(true);
    state = await context.storageState();
    const created = await context.request.post('/api/admin/entries', { headers: { Origin: baseURL! }, data: { kind: 'article', title: 'Building reliable software' } });
    expect(created.ok()).toBe(true);
    editorEntry = await created.json() as Entry;
    const saved = await context.request.patch(`/api/admin/entries/${editorEntry.id}`, {
      headers: { Origin: baseURL! },
      data: { version: editorEntry.version, content: { ...editorEntry.content, excerpt: 'Notes on clear boundaries, useful tests, and small changes.', body: '# Building reliable software\n\nGood software starts with clear boundaries and small, deliberate changes.\n\n## Start with the behavior\n\n- Keep a useful regression test.\n- Make one change at a time.\n- Review the result on a real screen.\n\n```csharp\npublic sealed record Note(string Title, string Body);\n```' } },
    });
    expect(saved.ok()).toBe(true);
    editorEntry = await saved.json() as Entry;
    const png = await sharp({ create: { width: 96, height: 64, channels: 3, background: '#e9dfc9' } }).png().toBuffer();
    const uploaded = await context.request.post('/api/admin/media', { headers: { Origin: baseURL! }, multipart: { file: { name: 'editorial-sample.png', mimeType: 'image/png', buffer: png }, alt: 'A neutral sample image for UI tests' } });
...
test.afterAll(async ({ browser, baseURL }) => {
  ...
  try {
    if (image) await context.request.delete(`/api/admin/media/${image.id}`, { headers: { Origin: baseURL! } });
    if (editorEntry) {
      const response = await context.request.get(`/api/admin/entries/${editorEntry.id}`);
      const latest = await response.json() as Entry;
      await context.request.post(`/api/admin/entries/${latest.id}/action`, { headers: { Origin: baseURL! }, data: { action: 'trash', version: latest.version } });
    }
  } finally { await context.close(); }
});
...
test('English admin screens fit five widths in both themes', async ({ page, browser, baseURL }, testInfo) => {
  const errors: string[] = [];
  page.on('pageerror', (error) => errors.push(error.message));
  const routes = ['/admin', '/admin/articles', '/admin/projects', '/admin/media', '/admin/taxonomies', '/admin/about', '/admin/settings', `/admin/articles/${editorEntry.id}`];
  ...
  for (const theme of ['light', 'dark']) {
    ...
    for (const width of [320, 375, 768, 1024, 1440]) {
      ...
      for (const route of routes) {
        ...
        expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1), `${route} ${width}px ${theme}`).toBe(true);
        if ([375, 1440].includes(width) && ['/admin', '/admin/media', '/admin/about', `/admin/articles/${editorEntry.id}`].includes(route)) {
          const name = route.endsWith(editorEntry.id) ? 'editor' : route === '/admin' ? 'overview' : route.split('/').at(-1);
          await testInfo.attach(`admin-${name}-${theme}-${width}`, { body: await page.screenshot({ fullPage: true }), contentType: 'image/png' });
        }
      }
    }
  }
  ...
  try {
    ...
    expect(await login.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1)).toBe(true);
    await testInfo.attach('login-mobile', { body: await login.screenshot({ fullPage: true }), contentType: 'image/png' });
...
test('filters persist in the URL and dashboard links cover both content kinds', async ({ page }) => {
  ...
  await page.reload();
  await expect(page.getByRole('searchbox', { name: 'Search articles' })).toHaveValue('unmatched-search-term');
  await ready(page, '/admin');
  for (const [title, status] of [['Drafts', 'draft'], ['Trash', 'trash']]) {
    const card = page.locator('.admin-stat').filter({ has: page.getByText(title, { exact: true }) });
    await expect(card.getByRole('link', { name: 'Articles' })).toHaveAttribute('href', `/admin/articles?status=${status}`);
    await expect(card.getByRole('link', { name: 'Projects' })).toHaveAttribute('href', `/admin/projects?status=${status}`);
  }
});

test('editor view, save shortcut, and recovery decision do not publish or erase drafts', async ({ page }) => {
  ...
  await expect(page.locator('.admin-markdown-input')).toBeVisible();
  await expect(page.getByRole('region', { name: 'Content preview' })).toBeVisible();
  const local = { content: { ...current.content, title: 'Keep this local recovery draft' }, at: new Date().toISOString(), version: current.version };
  await page.evaluate(({ id, local }) => localStorage.setItem(`kaiyo-draft-${id}`, JSON.stringify(local)), { id: editorEntry.id, local });
  await page.reload();
  ...
  expect(await page.evaluate((id) => JSON.parse(localStorage.getItem(`kaiyo-draft-${id}`)!).content.title, editorEntry.id)).toBe(local.content.title);
...
test('settings warn before sign-out and retain edits typed during a save', async ({ page, baseURL }) => {
  ...
  try {
    ...
    page.on('request', (request) => { if (request.url().endsWith('/api/auth/sign-out')) signOutRequests++; });
    ...
    release();
    await expect(page.getByLabel('Tagline', { exact: true })).toHaveValue('A newer edit made during save');
    await expect(page.getByRole('status').filter({ hasText: 'You still have unsaved changes.' })).toBeVisible();
    ...
    release(); await page.unroute('**/api/admin/settings');
    await page.request.put('/api/admin/settings', { headers: { Origin: baseURL! }, data: original });
  }
});
...
test('media search is keyboard accessible and resettable', async ({ page }) => {
  ...
  await expect(page.locator('.admin-media-card').filter({ hasText: 'editorial-sample.png' })).toBeVisible();
