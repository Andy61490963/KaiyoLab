...
const body = `A reading-layout fixture, not production content.\n\n## 前言\n\n` +
  '先釐清註冊、解析與生命週期，再討論實作的選擇。 '.repeat(30) +
  '\n\n## 註冊多個實作\n\n```csharp\n' + 'LongTypeName'.repeat(45) + '\n```\n\n' +
  '| Field | Value |\n| --- | --- |\n| Long value | ' + 'value'.repeat(120) + ' |\n\n' +
...
test.beforeAll(async ({ playwright, baseURL }) => {
  ...
  for (let attempt=0;attempt<3;attempt++) {
    const response = await owner.post('/api/auth/sign-in/email', {data:{
      email:process.env.E2E_EMAIL || 'e2e@example.test',
      password:process.env.E2E_PASSWORD || 'KaiyoLab-e2e-password-2026',
    }});
    if (response.status() !== 429 || attempt === 2) { expect(response.ok()).toBe(true); break; }
  ...
  expect(created.status()).toBe(201); entry = await created.json();
  const updated = await owner.patch(`/api/admin/entries/${entry.id}`,{data:{version:entry.version,content:{
    ...emptyContent,title,slug,body,excerpt:'從單一注入、IEnumerable 到 enum resolver，釐清註冊順序與生命週期。',
    category:'Backend Engineering',tags:['C#','ASP.NET Core','DI'],cover:'/images/cover-grid.svg',coverAlt:'Existing geometric test cover',
  }}});
  expect(updated.ok()).toBe(true); entry = await updated.json();
  const published = await owner.post(`/api/admin/entries/${entry.id}/action`, {data:{version:entry.version,action:'publish'}});
  expect(published.ok()).toBe(true); entry = await published.json();
});
test.afterAll(async () => {
  if (entry && owner) {
    const current = await (await owner.get(`/api/admin/entries/${entry.id}`)).json() as Entry;
    await owner.post(`/api/admin/entries/${entry.id}/action`,{data:{version:current.version,action:'trash'}});
  }
  await owner?.dispose();
});

test('views are persistent, reload-suppressed, localized, and not a unique-person claim', async ({page, request}) => {
...
test('article hero and reading columns fit both themes and languages without document overflow', async ({page}, testInfo) => {
  ...
  for (const theme of ['light','dark'] as const) {
    ...
    for (const width of [320,375,768,1024,1440]) {
      ...
      for (const language of ['English','繁體中文']) {
        ...
        await page.locator('.article-cover img').evaluate(async (image:HTMLImageElement)=>image.decode());
        expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),`${width}/${theme}/${language}`).toBe(true);
        if(width===1440) {
        ...
        } else if(width<=1024) { await expect(page.locator('.mobile-toc')).toBeVisible(); }
        if([375,1440].includes(width) && language==='繁體中文') {
          await expect(page.locator('[data-article-views]')).toHaveAttribute('data-view-state','ready');
          await page.screenshot({path:testInfo.outputPath(`article-${theme}-${width}.png`),fullPage:true,animations:'disabled'});
        }
      }
    }
  }
...
test('active table of contents follows sections, and native keyboard anchors still work', async ({page}) => {
  await page.goto(`/articles/${slug}`);
  ...
  await next.click(); await expect(next).toHaveAttribute('aria-current','location');
  await page.setViewportSize({width:375,height:812});
  await page.goto(`/articles/${slug}`);
  await page.locator('.mobile-toc summary').click();
  await page.getByRole('navigation',{name:'Mobile article table of contents'}).getByRole('link',{name:'註冊多個實作',exact:true}).click();
  expect(decodeURIComponent(new URL(page.url()).hash)).toBe('#註冊多個實作');
});

test('missing covers and failed telemetry do not break reading or invent counts', async ({page}) => {
  await page.route('**/api/article-views/*',route=>route.fulfill({status:503,contentType:'application/json',body:'{"error":"unavailable"}'}));
  ...
  await expect(page.locator('[data-article-views]')).toHaveAttribute('data-view-state','unavailable');
  await expect(page.locator('[data-view-number]')).toHaveText('—');
});

test('reading, original contents, and section links remain available without JavaScript', async ({browser,baseURL,request}) => {
  const before=await count(request,entry.id);
  const context=await browser.newContext({baseURL,javaScriptEnabled:false,viewport:{width:375,height:812}});
  try {
    const page=await context.newPage();await page.goto(`/articles/${slug}`);
    await expect(page.locator('main h1')).toHaveText(title);
    await page.locator('.mobile-toc summary').click();
    await page.getByRole('navigation',{name:'Mobile article table of contents'}).getByRole('link',{name:'註冊多個實作',exact:true}).click();
    expect(await count(request,entry.id)).toBe(before);
