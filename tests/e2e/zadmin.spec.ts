...
async function requestJson<T>(client: APIRequestContext, baseURL: string, route: string, method = 'GET', data?: unknown): Promise<T> {
...
test.beforeAll(async ({ request, baseURL }) => {
  ...
  if (new URL(response.url()).pathname === '/setup') {
    ...
    const setup = await request.post('/api/setup', { headers: { Origin: baseURL! }, data: { token: process.env.SETUP_TOKEN, email, password, name: 'Kaiyo', siteName: 'KaiyoLab' } });
...
test('categories and tags can be created, renamed, and deleted', async ({ page }) => {
  const suffix = Date.now(); const category = `驗收分類-${suffix}`; const renamed = `重新命名-${suffix}`; const tag = `驗收標籤-${suffix}`;
  await page.goto('/admin/taxonomies');
  const categories = page.locator('section.admin-panel').filter({ has: page.getByRole('heading', { name: 'Category', exact: true }) });
  const tags = page.locator('section.admin-panel').filter({ has: page.getByRole('heading', { name: 'Tag', exact: true }) });
  ...
  const originalRow = categories.locator('.admin-taxonomy-list > div').filter({ has: page.getByText(category, { exact: true }) });
  ...
  const renamedRow = categories.locator('.admin-taxonomy-list > div').filter({ has: page.getByText(renamed, { exact: true }) });
...
test('media upload, alt text, profile settings, and in-use image protection', async ({ page, baseURL }) => {
  ...
  try {
    ...
    await page.locator('input[type=file]').setInputFiles({ name: '不支援的圖片.svg', mimeType: 'image/svg+xml', buffer: Buffer.from('<svg xmlns="http://www.w3.org/2000/svg"><rect width="10" height="10"/></svg>') });
    await expect(page.getByRole('alert')).toContainText('Only PNG, JPEG, and WebP images are supported.');
    const buffer = await sharp({ create: { width: 96, height: 96, channels: 3, background: '#74e2ce' } }).png().toBuffer();
    await page.locator('input[type=file]').setInputFiles({ name: fileName, mimeType: 'image/png', buffer });
    ...
    await expect(card.getByRole('button', { name: `Delete image ${fileName}`, exact: true })).toBeDisabled();
    await card.locator('summary').click();
    await expect(card.getByText('Site settings / About me', { exact: true })).toBeVisible();
    const rejected = await page.request.delete(`/api/admin/media/${media!.id}`, { headers: { Origin: baseURL! } });
    expect(rejected.status()).toBe(409);
    await requestJson(page.request, baseURL!, '/api/admin/settings', 'PUT', original);
    await page.reload();
    card = page.locator('.admin-media-card').filter({ hasText: fileName });
    await expect(card.getByRole('button', { name: `Delete image ${fileName}`, exact: true })).toBeEnabled();
    page.once('dialog', (dialog) => dialog.accept());
    await card.getByRole('button', { name: `Delete image ${fileName}`, exact: true }).click();
    await expect(card).toHaveCount(0); media = undefined;
  } finally {
    await requestJson(page.request, baseURL!, '/api/admin/settings', 'PUT', original);
    if (media) await page.request.delete(`/api/admin/media/${media.id}`, { headers: { Origin: baseURL! } });
  }
});

test('failed saves recover and multi-tab conflicts preserve a draft copy', async ({ page, context, baseURL }) => {
  const created = await requestJson<Entry>(page.request, baseURL!, '/api/admin/entries', 'POST', { kind: 'article', title: `衝突驗收-${Date.now()}` });
  const route = `/admin/articles/${created.id}`; const endpoint = `/api/admin/entries/${created.id}`;
  const firstTitle = '先送出並保留的文章'; const localTitle = '斷線期間尚未送出的文章';
  await page.goto(route);
  await expect(page.getByLabel('Article title')).toHaveValue(created.content.title);
  await page.route(`**${endpoint}`, (intercepted) => intercepted.request().method() === 'PATCH' ? intercepted.fulfill({ status: 503, contentType: 'application/json', body: JSON.stringify({ error: '驗收用暫時離線，請重試。' }) }) : intercepted.continue());
  ...
  await expect(second.getByText('This content was changed in another tab', { exact: true })).toBeVisible();
  ...
  await expect(second.getByLabel('Article title')).toHaveValue(`${conflictingTitle} (recovered copy)`);
  ...
  expect(copy.published).toBeNull(); expect(copy.content.title).toBe(`${conflictingTitle} (recovered copy)`);
  second.once('dialog', (dialog) => dialog.accept());
  await second.getByRole('button', { name: 'Move to trash', exact: true }).click();
  await expect(second.getByText('Moved to trash. You can restore it at any time.', { exact: true })).toBeVisible();
  await second.getByRole('button', { name: 'Restore content', exact: true }).click();
  await expect(second.getByText('Restored as a draft. Review the content before publishing again.', { exact: true })).toBeVisible();
  for (const id of [created.id, copyId]) {
    const latest = await requestJson<Entry>(page.request, baseURL!, `/api/admin/entries/${id}`);
    await requestJson(page.request, baseURL!, `/api/admin/entries/${id}/action`, 'POST', { action: 'trash', version: latest.version });
  }
  await second.close();
});
