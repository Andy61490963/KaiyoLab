...
const noteSlugs = [
  'dotnet-di-multiple-implementations',
  'tls-certificate-name-mismatch',
  'debugging-502-container-vs-public',
  'draft-autosave-publish-boundaries',
  'mes-integration-before-retry',
];
const authoredHtml = (page: Page) =>
  page.locator('[data-authored-content]').evaluate((element) => {
...
test('toggle changes only UI and persists without navigating or rewriting content', async ({
  page,
  context,
}) => {
  const errors: string[] = [];
...
test('public refinements fit both languages and themes at five viewport widths', async ({
  page,
}, testInfo) => {
...
test('Chinese search and mobile controls retain their behaviors', async ({ page }) => {
  ...
  await page
    .getByRole('navigation', { name: '行動版導覽' })
  ...
  await expect(
    page.getByRole('heading', { name: 'MES 系統整合：Timeout 之後，重試之前' }),
  ).toBeVisible();
...
test('blocked preference storage and reduced motion keep the switch usable', async ({ page }) => {
  ...
  await expect(page.locator('html')).toHaveAttribute('lang', 'zh-TW');
  expect(
    await languageButton(page).evaluate(
      (button) => getComputedStyle(button, '::before').transitionDuration,
    ),
  ).toBe('0s');
...
test('a long, authored timeline keeps its text with the cat beside it', async ({
  page,
  baseURL,
}, testInfo) => {
  ...
  try {
    const update = await page.request.put('/api/admin/settings', {
      headers,
      data: { ...original, authorName: 'Andy', homeIntro: intro },
    });
