...
const noteSlugs = ['aspnet-core-di-multiple-implementations', 'tls-name-mismatch-file-sync', '502-follow-the-request', 'mes-timeouts-idempotency', 'manufacturing-ai-poc-evaluation'];

test('language changes UI without navigating, rewriting content, or losing search state', async ({ page }) => {
  await page.goto('/');
  ...
  await chinese.focus(); await page.keyboard.press('Space');
...
test('all five notes are real published content and keep their original language', async ({ page, request }) => {
...
test('home layout, portrait, and controls fit both languages in both themes', async ({ page, request, baseURL }, testInfo) => {
  ...
  try {
    ...
    for (const theme of ['light', 'dark'] as const) {
      ...
      for (const width of [320, 375, 768, 900, 1024, 1440]) {
        ...
        for (const language of ['en', 'zh-TW'] as const) {
          await page.goto('/');
          await page.getByRole('button', { name: language === 'en' ? 'English' : '繁體中文', exact: true }).click();
          await page.locator('.home-mascot img').evaluate(async (img: HTMLImageElement) => { await img.decode(); });
          ...
          if ([375, 1440].includes(width)) {
            await page.screenshot({ path: testInfo.outputPath(`home-${theme}-${language}-${width}.png`), fullPage: true, animations: 'disabled' });
          }
        }
      }
    }
    ...
    expect((await request.put('/api/admin/settings', { headers: { Origin: baseURL! }, data: original })).ok()).toBeTruthy();
  }
});

test('language selection synchronizes tabs and does not interfere with themes or menus', async ({ page, context }) => {
  await page.goto('/');
  const other = await context.newPage(); await other.goto('/about');
  await page.getByRole('button', { name: '繁體中文', exact: true }).click();
...
test('blocked storage and reduced motion keep the switch usable', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '繁體中文', exact: true }).click();
  await expect(page.locator('html')).toHaveAttribute('lang', 'zh-TW');
  expect(await page.locator('.language-switch-thumb').evaluate(element => getComputedStyle(element).transitionDuration)).toBe('0s');
  await page.getByRole('button', { name: '切換深色主題', exact: true }).click();
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'dark');
});

test('without JavaScript the default interface works and no dead language control is shown', async ({ browser, baseURL }) => {
