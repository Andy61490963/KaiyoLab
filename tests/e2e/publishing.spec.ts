...
test('owner setup, publishing, and responsive reading', async ({
  page,
  request,
  baseURL,
  context,
  ...
  for (const theme of ['dark', 'light'] as const) {
    ...
    for (const width of [375, 768, 1440]) {
      ...
      for (const route of [
        ...
        expect(
          ...
          `${route} ${theme} ${width}px should not overflow`,
        ).toBeTruthy();
        ...
        if (process.env.CAPTURE_DOCS && route === '/projects' && (width === 1440 || width === 375))
    ...
    if (process.env.CAPTURE_DOCS && theme === 'dark') {
      ...
      await expect(page.getByText('Recently edited', { exact: true })).toBeVisible();
  ...
  await expect(
    page.getByText('Unpublished. The content is kept as a private draft.', { exact: true }),
  ).toBeVisible();
