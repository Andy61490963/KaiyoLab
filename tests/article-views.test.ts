...
describe('article view metrics', () => {
  ...
  it('accepts receipts only for the same article, key, and unexpired time window', () => {
    ...
    expect(
      validViewReceipt(receipt, 'article-a', 'another-key-0000000000000000000000000', now),
    ).toBe(false);
    expect(validViewReceipt(receipt, 'article-a', key, now + VIEW_COOLDOWN_SECONDS * 1000)).toBe(
      false,
    );
  ...
  it('rejects malformed or forged receipts without throwing', () => {
    for (const value of [
      ...
      '0.x',
      'x'.repeat(20000),
      createViewReceipt('a', key, now) + 'x',
    ])
      expect(validViewReceipt(value, 'a', key, now)).toBe(false);
    expect(
      validViewReceipt(createViewReceipt('a', key, now).replace(/.$/, '!'), 'a', key, now),
    ).toBe(false);
