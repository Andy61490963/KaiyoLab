...
function refreshLabels() {
  document.querySelectorAll<HTMLElement>('[data-ui-label]').forEach((element) => {
    if (isUiLabel(element.dataset.uiLabel))
      element.setAttribute('aria-label', uiLabel(element.dataset.uiLabel, language));
  });
  document.querySelectorAll<HTMLInputElement>('[data-ui-placeholder]').forEach((element) => {
    if (isUiLabel(element.dataset.uiPlaceholder))
      element.placeholder = uiLabel(element.dataset.uiPlaceholder, language);
  });
  document.querySelectorAll<HTMLElement>('[data-ui-message]').forEach((element) => {
    if (isUiLabel(element.dataset.uiMessage))
      element.textContent = uiLabel(element.dataset.uiMessage, language);
  });
  buttons.forEach((button) => {
    button.setAttribute('aria-pressed', String(language === 'zh-TW'));
    button.setAttribute(
      'aria-label',
      language === 'en' ? 'Switch interface language to Traditional Chinese' : '切換介面語言為英文',
    );
...
window.addEventListener('pageshow', () => {
  ...
  try {
    applyLanguage(localStorage.getItem(UI_LANGUAGE_KEY));
  } catch {
    refreshLabels();
  }
});
