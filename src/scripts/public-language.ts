...
export const currentUiText = (key: UiKey) => uiText(key, normalizeUiLanguage(document.documentElement.dataset.uiLanguage));
...
export function initializeUiLanguage() {
  ...
  const apply = (value: unknown, announce = false) => {
    ...
    for (const element of document.querySelectorAll<HTMLElement>('[data-ui-label-en]')) {
      ...
      element.setAttribute('aria-label', (language === 'en' ? element.dataset.uiLabelEn : element.dataset.uiLabelZh) || '');
    }
    for (const element of document.querySelectorAll<HTMLInputElement>('[data-ui-placeholder-en]')) {
      element.placeholder = (language === 'en' ? element.dataset.uiPlaceholderEn : element.dataset.uiPlaceholderZh) || '';
    }
    const title = document.querySelector<HTMLTitleElement>('title[data-ui-title-en]');
    if (title) title.textContent = (language === 'en' ? title.dataset.uiTitleEn : title.dataset.uiTitleZh) || '';
    ...
    if (announce) {
      ...
      if (status) status.textContent = currentUiText('Interface language: English. Content is unchanged.');
    }
  };
  ...
  control.querySelectorAll<HTMLButtonElement>('button[data-language]').forEach(button => {
    button.addEventListener('click', () => {
      ...
      apply(language, true);
      try { localStorage.setItem(UI_LANGUAGE_KEY, language); } catch { /* Still works for this page. */ }
    });
  });
  ...
  window.addEventListener('pageshow', () => {
    try { apply(localStorage.getItem(UI_LANGUAGE_KEY)); } catch { apply(root.dataset.uiLanguage); }
  });
}
