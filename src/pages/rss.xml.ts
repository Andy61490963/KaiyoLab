...
export const GET: APIRoute = async () => {
  ...
  return new Response(
    `<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>${x(s.siteName)}</title><link>${x(s.siteUrl)}</link><description>${x(s.description)}</description><language>en-US</language>${all.map((e) => `<item><title>${x(e.title)}</title><link>${x(new URL(`/articles/${encodeURIComponent(e.slug)}`, s.siteUrl).href)}</link><guid isPermaLink="false">${x(e.id)}</guid><description>${x(e.excerpt)}</description><pubDate>${new Date(e.publishedAt).toUTCString()}</pubDate></item>`).join('')}</channel></rss>`,
    {
      headers: {
